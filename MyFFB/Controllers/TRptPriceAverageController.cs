...
{
    public class TRptPriceAverageController : Controller
    {
        ...
        string classname = "TRptPriceAverage";
        private readonly IWebHostEnvironment _env;


        public TRptPriceAverageController(RequestHandler requestHandler, IWebHostEnvironment env)
        {
            _env = env;
        ...
        [HttpPost]
        public ActionResult Print([FromBody] T_Report.RptPriceAverage ParJson)
        {
            if (roledetobj == null)
            {
                return RedirectToAction("Index", new RouteValueDictionary(new { controller = "Home", action = "Index" }));
            ...
            else
            {
                ...
                string path = contentRootPath + "//Report//Rpt_PriceAverage.rdlc";
                var model = SReport.Report(econstr, jconstr).RptPriceAverage(juserid, ParJson);
                ...
                parameters.Add(new ReportParameter("Par1", "Price Average"));
